...
function win(winner, highlight) {
  ...
  if (winner === undefined) message.innerHTML = "<h3>무승부입니다</h3>";
    ...
    if (!spectate)
      (winner.id === socket.id ? myElement : opponentElement).classList.add(
        "win",
      );
    else
      [black, white][
    ...
    switch (type) {
      ...
      case "v":
        for (let y = hy; y <= hy + 4; y++)
          highlightElements.push(clickElements[y][hx]);
        break;
      case "d":
        for (let i = 0; i < 5; i++)
          highlightElements.push(clickElements[hy + i][hx + i]);
        break;
      case "rd":
        for (let i = 0; i < 5; i++)
          highlightElements.push(clickElements[hy + i][hx - i]);
    ...
    for (let row of clickElements) {
      row.forEach((e) => {
        ...
        if (!highlightElements.includes(e) && e.classList.contains("placed"))
          e.style.opacity = 0.5;
      });
    }
  }
