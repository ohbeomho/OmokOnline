...
class Room {
  ...
  place(x, y, user) {
    if (
      this.users.indexOf(user) !== this.turn ||
      x > 14 ||
      y > 14 ||
      x < 0 ||
      y < 0
  ...
  checkWin() {
    ...
    if (!this.board.find((v) => typeof v !== "number"))
...
io.on("connection", (socket) => {
    ...
    if (!targetRoom)
      ...
      if (spectate) {
        ...
        room.users.forEach((user) =>
          io.to(user.id).emit("game", { type: "join-spec" }),
        );
        socket.emit("room", {
          type: "room-info",
          room,
  ...
  socket.on("disconnect", () => {
    if (room) {
      if (room.users.length > 1 && !spec)
        socket.to(room.name).emit("game", { type: "disconnect", user });

      rooms.splice(rooms.indexOf(this), 1);
    } else if (spec) {
      room.users.forEach((user) =>
        io.to(user.id).emit("game", { type: "leave-spec" }),
      );
    }
  });
});
